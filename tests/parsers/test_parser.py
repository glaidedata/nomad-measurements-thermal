...
def test_is_mainfile():
    ...
    assert parser.is_mainfile("test_file.dat", "text/plain", valid_content.encode(), valid_content)

    # Invalid file structure
    invalid_content = "Just some random text without the proper headers."
    assert not parser.is_mainfile("test_file.dat", "text/plain", invalid_content.encode(), invalid_content)

@patch('nomad_measurements_thermal.schema_packages.schema_package.ThermalMeasurement.normalize')
